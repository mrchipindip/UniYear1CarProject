...
public class PlayerCar2 : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		else
		{
			...
			{
				m_appliedTorque = engineTorque / gearRatio[m_currentGear] * Input.GetAxis("Vertical2");
			}
			else
			{
		...
		frontLeftWheel.steerAngle = 25.0f * Input.GetAxis("Horizontal2");
		frontRightWheel.steerAngle = 25.0f * Input.GetAxis("Horizontal2");
	}
